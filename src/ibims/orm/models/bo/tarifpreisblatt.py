...
class Tarifpreisblatt(SQLModel, table=True):
    """
    ...
    """
    tarifpreisblatt_sqlid: uuid_pkg.UUID = Field(
        default_factory=uuid_pkg.uuid4, primary_key=True, index=True, nullable=False
    )
    typ: Typ | None = Field(Typ.TARIFPREISBLATT)
    ...
    anbieter: "Marktteilnehmer" = Relationship(
        back_populates="tarifpreisblatt_anbieter",
        sa_relationship_kwargs={"foreign_keys": "[Tarifpreisblatt.anbieter_id]"},
    )
    ...
    berechnungsparameter: "Tarifberechnungsparameter" = Relationship(
        back_populates="tarifpreisblatt_berechnungsparameter",
        sa_relationship_kwargs={"foreign_keys": "[Tarifpreisblatt.berechnungsparameter_id]"},
    )
    ...
    energiemix: "Energiemix" = Relationship(
        back_populates="tarifpreisblatt_energiemix",
        sa_relationship_kwargs={"foreign_keys": "[Tarifpreisblatt.energiemix_id]"},
    )
    ...
    preisgarantie: "Preisgarantie" = Relationship(
        back_populates="tarifpreisblatt_preisgarantie",
        sa_relationship_kwargs={"foreign_keys": "[Tarifpreisblatt.preisgarantie_id]"},
    )
    registeranzahl: Registeranzahl | None = Field(None)
    sparte: Sparte | None = Field(None)
    tarifAufAbschlaege: List["AufAbschlag"] = Relationship(
        back_populates="tarifpreisblatt_tarifaufabschlaege_link",
    ...
    tarifeinschraenkung: "Tarifeinschraenkung" = Relationship(
        back_populates="tarifpreisblatt_tarifeinschraenkung",
        sa_relationship_kwargs={"foreign_keys": "[Tarifpreisblatt.tarifeinschraenkung_id]"},
    )
    tarifmerkmale: List[Tarifmerkmal] | None = Field(
        None, sa_column=Column(ARRAY(Enum(Tarifmerkmal, name="tarifmerkmal")))
    )
    tarifpreise: List["Tarifpreisposition"] = Relationship(
        back_populates="tarifpreisblatt_tarifpreise_link",
        link_model=TarifpreisblatttarifpreiseLink,
    )
    ...
    vertragskonditionen: "Vertragskonditionen" = Relationship(
        back_populates="tarifpreisblatt_vertragskonditionen",
        sa_relationship_kwargs={"foreign_keys": "[Tarifpreisblatt.vertragskonditionen_id]"},
    )
    ...
    zeitlicheGueltigkeit: "Zeitraum" = Relationship(
        back_populates="tarifpreisblatt_zeitlicheGueltigkeit",
        sa_relationship_kwargs={"foreign_keys": "[Tarifpreisblatt.zeitlicheGueltigkeit_id]"},
    )
    zusatzAttribute: List["ZusatzAttribut"] = Relationship(
        back_populates="tarifpreisblatt_zusatzattribute_link",
        link_model=TarifpreisblattzusatzAttributeLink,
    )
