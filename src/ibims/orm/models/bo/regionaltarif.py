...
from ibims.orm.models.many import (
    RegionaltarifpreisgarantienLink,
...
class Regionaltarif(SQLModel, table=True):
    """
    .. raw:: html
    ...
    """
    regionaltarif_sqlid: uuid_pkg.UUID = Field(
        default_factory=uuid_pkg.uuid4, primary_key=True, index=True, nullable=False
    )
    typ: Typ | None = Field(Typ.REGIONALTARIF)
    ...
    anbieter: "Marktteilnehmer" = Relationship(
        back_populates="regionaltarif_anbieter",
        sa_relationship_kwargs={"foreign_keys": "[Regionaltarif.anbieter_id]"},
    )
    ...
    berechnungsparameter: "Tarifberechnungsparameter" = Relationship(
        back_populates="regionaltarif_berechnungsparameter",
        sa_relationship_kwargs={"foreign_keys": "[Regionaltarif.berechnungsparameter_id]"},
    )
    ...
    energiemix: "Energiemix" = Relationship(
        back_populates="regionaltarif_energiemix",
        sa_relationship_kwargs={"foreign_keys": "[Regionaltarif.energiemix_id]"},
    )
    kundentypen: List[Kundentyp] | None = Field(None, sa_column=Column(ARRAY(Enum(Kundentyp, name="kundentyp"))))
    preisgarantien: List["RegionalePreisgarantie"] = Relationship(
        back_populates="regionaltarif_preisgarantien_link",
        link_model=RegionaltarifpreisgarantienLink,
    )
    registeranzahl: Registeranzahl | None = Field(None)
    sparte: Sparte | None = Field(None)
    tarifAufAbschlaege: List["RegionalerAufAbschlag"] = Relationship(
        back_populates="regionaltarif_tarifaufabschlaege_link",
    ...
    tarifeinschraenkung: "Tarifeinschraenkung" = Relationship(
        back_populates="regionaltarif_tarifeinschraenkung",
        sa_relationship_kwargs={"foreign_keys": "[Regionaltarif.tarifeinschraenkung_id]"},
    )
    tarifmerkmale: List[Tarifmerkmal] | None = Field(
        None, sa_column=Column(ARRAY(Enum(Tarifmerkmal, name="tarifmerkmal")))
    )
    tarifpreise: List["RegionaleTarifpreisposition"] = Relationship(
        back_populates="regionaltarif_tarifpreise_link",
        link_model=RegionaltariftarifpreiseLink,
    )
    ...
    vertragskonditionen: "Vertragskonditionen" = Relationship(
        back_populates="regionaltarif_vertragskonditionen",
        sa_relationship_kwargs={"foreign_keys": "[Regionaltarif.vertragskonditionen_id]"},
    )
    ...
    zeitlicheGueltigkeit: "Zeitraum" = Relationship(
        back_populates="regionaltarif_zeitlicheGueltigkeit",
        sa_relationship_kwargs={"foreign_keys": "[Regionaltarif.zeitlicheGueltigkeit_id]"},
    )
    zusatzAttribute: List["ZusatzAttribut"] = Relationship(
        back_populates="regionaltarif_zusatzattribute_link",
        link_model=RegionaltarifzusatzAttributeLink,
    )
