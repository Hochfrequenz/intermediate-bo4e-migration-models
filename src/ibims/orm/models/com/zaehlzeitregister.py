...
from ibims.orm.models.many import ZaehlzeitregisterzusatzAttributeLink
...
class Zaehlzeitregister(SQLModel, table=True):
    """
    ...
    Zählzeitdefinition
    """
    zaehlzeitregister_sqlid: uuid_pkg.UUID = Field(
        default_factory=uuid_pkg.uuid4, primary_key=True, index=True, nullable=False
    )
    zaehlwerk_zaehlzeitregister: List["Zaehlwerk"] = Relationship(
        back_populates="zaehlzeitregister",
        sa_relationship_kwargs={
            "primaryjoin": "Zaehlwerk.zaehlzeitregister_id==Zaehlzeitregister.zaehlzeitregister_sqlid",
            "lazy": "joined",
        },
    )
    zusatzAttribute: List["ZusatzAttribut"] = Relationship(
        back_populates="zaehlzeitregister_zusatzattribute_link",
