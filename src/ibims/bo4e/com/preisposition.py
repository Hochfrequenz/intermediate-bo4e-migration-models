...
class Preisposition(BaseModel):
    ...
    """
    Eine vom BDEW standardisierte Bezeichnug fÃ¼r die abgerechnete Leistungserbringung;
    ...
    """
    Hier wird festgelegt, auf welche BezugsgrÃ¶sse sich der Preis bezieht, z.B. kWh oder StÃ¼ck
    """
    freimenge_blindarbeit: Optional[Decimal] = Field(
        default=None, alias="freimengeBlindarbeit", title="Freimengeblindarbeit"
    )
    """
    Der Anteil der Menge der Blindarbeit in Prozent von der Wirkarbeit, fÃ¼r die keine Abrechnung erfolgt
    """
    ...
    """
    Der cos phi (VerhÃ¤ltnis Wirkleistung/Scheinleistung) aus dem die Freimenge fÃ¼r die Blindarbeit berechnet wird als
    tan phi (VerhÃ¤ltnis Blindleistung/Wirkleistung)
    """
    gruppenartikel_id: Optional[str] = Field(default=None, alias="gruppenartikelId", title="Gruppenartikelid")
    """
    Ãœbergeordnete Gruppen-ID, die sich ggf. auf die Artikel-ID in der Preisstaffel bezieht
    """
    leistungsbezeichnung: Optional[str] = Field(default=None, title="Leistungsbezeichnung")
    """
    Bezeichnung fÃ¼r die in der Position abgebildete Leistungserbringung
    """
    leistungstyp: Optional[Leistungstyp] = None
    """
    Standardisierte Bezeichnung fÃ¼r die abgerechnete Leistungserbringung
    """
    preiseinheit: Optional[Waehrungseinheit] = None
    """
    Festlegung, mit welcher Preiseinheit abgerechnet wird, z.B. Ct. oder â‚¬
    """
    preisstaffeln: list["Preisstaffel"] = Field(..., title="Preisstaffeln")
    """
    Preisstaffeln, die zu dieser Preisposition gehÃ¶ren
    """
    tarifzeit: Optional[Tarifzeit] = None
    """
    Festlegung, fÃ¼r welche Tarifzeit der Preis hier festgelegt ist
    """
    zeitbasis: Optional[Mengeneinheit] = None
    """
    Die Zeit(dauer) auf die sich der Preis bezieht.
    Z.B. ein Jahr fÃ¼r einen Leistungspreis der in â‚¬/kW/Jahr ausgegeben wird
    """
    zonungsgroesse: Optional[Bemessungsgroesse] = None
    """
    Mit der Menge der hier angegebenen GrÃ¶ÃŸe wird die Staffelung/Zonung durchgefÃ¼hrt. Z.B. Vollbenutzungsstunden
    """
